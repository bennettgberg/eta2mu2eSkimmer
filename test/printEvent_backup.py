...
def printEvent(entry, isMC=False, gentry=None) :
    ...
    if len(entry.Vertex_mumu_dR) > 0:
        print("Vertices mu+mu-\n #      vx       vy       vz     sigmaVxy    reduced_chi2     dR     mu+     mu-")
        for j in range(len(entry.Vertex_mumu_dR)):
            print("{0:d}  {1:.3f}    {2:.3f}     {3:.3f}     {4:.3f}      {5:.3f}    {6:.3f}      {7:d}      {8:d}".format(
                j,entry.Vertex_mumu_vx[j], entry.Vertex_mumu_vy[j], entry.Vertex_mumu_vz[j], entry.Vertex_mumu_sigmaVxy[j],
                entry.Vertex_mumu_reduced_chi2[j], entry.Vertex_mumu_dR[j], 
                ord(entry.Vertex_mumu_muP[j]), ord(entry.Vertex_mumu_muN[j])
                ))

    if len(entry.Vertex_elel_dR) > 0:
        print("Vertices mu+mu-\n #      vx       vy       vz     sigmaVxy    reduced_chi2    ndof    dR     el+     el-")
        for j in range(len(entry.Vertex_elel_dR)):
            print("{0:d}  {1:.3f}    {2:.3f}     {3:.3f}     {4:.3f}      {5:.3f}      {6:.3f}      {7:d}      {8:d}".format(
                ...
                entry.Vertex_elel_reduced_chi2[j], entry.Vertex_elel_dR[j], 
                ord(entry.Vertex_elel_eleP[j]), ord(entry.Vertex_elel_eleN[j])
                ))
    if isMC:
